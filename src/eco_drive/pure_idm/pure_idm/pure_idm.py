...
class SimpleProportionalController(Node):
    ...
    def __init__(self):
        ...
        self.gap = 26.0  # initial large gap
        ...
        self.idm_params = {
            'v0': 13.0,      # Desired speed (m/s) - 36 km/h
            'T': 1.20,       # Safe time headway (s) - AV aggressive
            's0': 15.0,       # Minimum spacing (m)
            'delta': 4.0,    # Acceleration exponent
            'a': 3.00,       # Maximum acceleration (m/s²)
            'b': 3.5         # Comfortable deceleration (m/s²) - Max decel limit
        }
